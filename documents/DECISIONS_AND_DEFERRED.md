...
- **Live `browser_cdp` `Runtime.evaluate` calls have been unreliable this session:** one call stalled for 30+ minutes and had to be manually cancelled (root cause undetermined — possibly `Emulation.setEmulatedMedia` interacting badly with a subsequent `Runtime.evaluate`, possibly an orphaned `npx serve`/stray Chrome process contention, never fully isolated).
- **Stale `Emulation.setEmulatedMedia` override bled into unrelated later screenshots:** a `prefers-reduced-motion: reduce` override set during a `/vision` static-fallback check was never cleared, and silently stayed active through the entire M2 Button and nav-tap-target verification screenshots on the same tab. This made `VideoBackground.tsx`'s legitimate reduced-motion fallback (static `Mycelia_Interactive_Logo.jpg` poster) appear in every subsequent screenshot in place of the real animated spritesheet, and was initially mistaken for a production asset bug — it was purely leftover local tool state, confirmed via `window.matchMedia('(prefers-reduced-motion: reduce)').matches` still returning `true` on the same tab well after the original check. Root cause of adopting the "Verification method policy" below.

## Verification method policy

- **Default verification method:** the four automated checks (`npm run build`, `npm run lint`, `npx tsc --noEmit`, `npx vitest run`) plus reading compiled output/source directly (grep the built HTML/CSS/JS in `out/`). This is sufficient for the vast majority of steps and requires no browser automation at all.
- **The live browser tool is OFF by default.** Only use it when a fix genuinely cannot be verified any other way (e.g., visual/animation behavior that can't be confirmed by reading code or compiled output), and only for the single, minimal check needed — one page, one action, then stop.
- **Any browser tool use must be proposed and approved by name before it happens** — never silently bundled into a general "verification" step.
- **Never leave a CDP/emulation override active.** If `Emulation.setEmulatedMedia`, `Emulation.setDeviceMetricsOverride`, or similar is ever used, clear it in the same response that set it, before moving to any other task — do not wait until it's noticed as a problem later.
